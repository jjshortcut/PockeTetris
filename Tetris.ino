/* 2015 / 2016 /2017
 *  
 *  JJ: Attiny85 @8mhz low fuse at E2
 *  Resistors are pulldown so: PLUS-pushbutton-pin-resistor(1K)-GND
...
void loop() {
  ...
  */
  ...
  ssd1306_char_f8x8(1, 48, " VOOR");
  ssd1306_char_f8x8(1, 40, "LOTJE");
  ...
  if (sChange == 0) {
    ...
    ssd1306_char_f8x8(1, 20, "VAN");
    ssd1306_char_f8x8(1, 10, "J-J");
    delay(1500);
    ssd1306_fillscreen(0x00);
    playTetris();
  }
...
void handleInput(void) {
  ...
  if (digitalRead(2) == HIGH && keyLock == 2 && millis() - keyTime > 300) {
    while (digitalRead(2) == HIGH) {  // move down
      drawPiece(ERASE);
      movePieceDown();
      drawPiece(DRAW);
  ...
  //add 3rd button - left button, move left
  ...
  // Pause the game, left and right together
  if ((digitalRead(0) == HIGH) && (digitalRead(1) == HIGH))
  {
    //ssd1306_fillscreen(0x00);
    ssd1306_char_f8x8(0, 117, "PAUSED");
    delay(800); // debounce
    uint8_t timeout = 100;
    while((digitalRead(0) == LOW) && (digitalRead(1) == LOW) && (digitalRead(2) == LOW) && (timeout != 0)) // while nothing pressed
    {
      timeout--;
      
      if(timeout == 30)
      {
       ssd1306_char_f8x8(0, 117, "SLP 3 "); // going to sleep
      }
      if(timeout == 20)
      {
       ssd1306_char_f8x8(4, 117, "2"); // going to sleep
      }
      if(timeout == 10)
      {
       ssd1306_char_f8x8(4, 117, "1"); // going to sleep
      }
      ...
      if(timeout == 0) 
      {
        //ssd1306_char_f8x8(0, 117, "S"); // going to sleep
        system_sleep();  // go to sleep
        drawGameScreen(0, 10, 0, VERTDRAW, FULL);
      }
      // Wait for a keypress.. or aftersleep
    }
    displayScore(score, 0, 117, 0);
}
  ...
  if (digitalRead(0) == LOW && digitalRead(1) == LOW && digitalRead(2) == LOW) {
    if (keyLock == 2  && millis() - keyTime < 300) {  // rotate piece = 2
      drawPiece(ERASE);
      rotatePiece();
      drawPiece(DRAW);
      drawGameScreen(currentPiece.column, currentPiece.column + 4, currentPiece.row, currentPiece.row + 4, PARTIAL);
    } else if (keyLock == 1) { // right move = 1
      drawPiece(ERASE);
      movePieceRight();
      drawPiece(DRAW);
      drawGameScreen(currentPiece.column - 1, currentPiece.column + 4, currentPiece.row, currentPiece.row + 4, PARTIAL);

    } else if (keyLock == 3) { //3rd button // left move = 3
      drawPiece(ERASE);
      movePieceLeft();
      drawPiece(DRAW);
      drawGameScreen(currentPiece.column, currentPiece.column + 5, currentPiece.row, currentPiece.row + 4, PARTIAL);
